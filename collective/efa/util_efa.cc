...
EFASocket::EFASocket(int gpu_idx, int dev_idx, int socket_idx)
  ...
  // Intel MEV NIC does not support GPUDirect. So ibv_reg_mr will fail with cudaMalloc
  // cudaMallocManaged allocates a unified single virtual address range that is accessible
  // from both CPU and GPU. Runtime migrates pages on demand between host and device when
  // either side accesses them, using page faults to trigger migration.
  auto cuda_ret = cudaMallocManaged(&pkt_data_buf_, PktDataBuffPool::kNumPktData *
                                               PktDataBuffPool::kPktDataSize,cudaMemAttachGlobal);
