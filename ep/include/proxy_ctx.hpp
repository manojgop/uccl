...
struct ProxyCtx {
  ...
  // size limit (e.g. irdma DMA-BUF 2 GiB limit).  When empty, the single
  // ctx.mr / ctx.rkey / ctx.remote_rkey are used directly.
  ...
  uint32_t lkey_for(uintptr_t addr) const {
    for (auto& c : gpu_mr_chunks) {
      if (addr >= c.base && addr < c.base + c.len)
        return c.mr->lkey;
    }
  ...
  std::vector<WRSegment> split_for_chunks(
      uintptr_t laddr, uintptr_t raddr, uint32_t total_len) const {
